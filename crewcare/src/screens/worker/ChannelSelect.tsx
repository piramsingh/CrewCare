...
/**
 * Channel hand-off. In production the worker taps a link and the conversation
 * continues in their own messaging app; in the demo both choices open the same
 * simulated thread, skinned to match.
 */
export function ChannelSelect({
  ...
  const [demoCode] = useState(() =>
    Math.random().toString(36).slice(2, 6).toUpperCase() + '-' + Math.random().toString(36).slice(2, 5).toUpperCase(),
  )
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        <span className="block text-[16px] font-bold text-cc-primary">Continue on {theme.label}</span>
        <span className="block text-[13px] text-cc-grey">{theme.blurb}</span>
      </span>
      <svg viewBox="0 0 8 14" className="ml-auto h-3.5 w-2 shrink-0 fill-none stroke-cc-grey stroke-2">
        <path d="M1 1l6 6-6 6" />
      </svg>
    </button>
  )
