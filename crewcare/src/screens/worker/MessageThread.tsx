...
import { PUSH_PREVIEW } from '../../conversation/script'
...
export function MessageThread({
  ...
  // Shown once, a beat after the worker opts into alerts.
  const [push, setPush] = useState<'pending' | 'showing' | 'done'>('pending')
  ...
  // A worker who opted in sees what an alert actually looks like on their
  // lock screen. Declining shows nothing — there would be nothing to show.
  const alerts = state.answers.notify
  const optedIn = Boolean(alerts && !alerts.skipped && alerts.optionId !== 'none')
  useEffect(() => {
    if (!optedIn || push !== 'pending') return
    const timer = setTimeout(() => setPush('showing'), 900)
    return () => clearTimeout(timer)
  }, [optedIn, push])
  ...
  return (
    ...
    >
      ...
      {push === 'showing' && (
        <PushNotification text={PUSH_PREVIEW} onDone={() => setPush('done')} />
      )}
